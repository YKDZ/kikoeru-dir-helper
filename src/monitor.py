...
class ArchiveEventHandler(FileSystemEventHandler):
    ...
    def _handle_new_file(self, file_path):
        ...
        try:
            ...
            file_mtime = file_path.stat().st_mtime if file_path.exists() else current_time
        except OSError:
            file_size = 0
        ...
        logging.info(f"文件 {file_path} 添加到待处理队列 ({file_size/1024/1024:.2f}MB)")
    ...
    def check_pending_files(self):
        ...
        for file_path_str, file_info in list(self.pending_files.items()):
            ...
            if file_info['stable_count'] >= self.min_stable_checks:
                ...
                logging.info(f"文件 {file_path} 等待中: {time_since_creation:.1f}s, 稳定次数: {file_info['stable_count']}/{self.min_stable_checks}")
            else:
                # 还在初始等待期
...
class DirectoryMonitor:
    ...
    def start(self):
        ...
        logging.info(f"⚙️ 监控配置: 检查间隔={self.check_interval}s, 稳定等待={self.event_handler.stability_wait_time}s, 最大等待={self.event_handler.max_wait_time}s")
