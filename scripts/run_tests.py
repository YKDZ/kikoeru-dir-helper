...
def discover_test_modules() -> Dict[str, str]:
    ...
    for test_file in tests_dir.glob("*.py"):
        if (
            ...
            or test_file.name.endswith("_tests.py")
        ):
    ...
    try:
        ...
        # 3. 尝试查找 test_ 开头的函数
            ...
            for name, obj in inspect.getmembers(module):
                if (
                    ...
                    and callable(obj)
                    and not name.startswith("test_case")
                ):
                    entry_function = obj
                    break
        ...
        try:
            ...
            if success:
                ...
                return TestResult(
                    module_name, description, False, f"测试返回错误码: {result}"
                )
            ...
            if success:
                ...
                print(f"❌ {description}失败")
                return TestResult(
                    module_name, description, False, f"SystemExit: {e.code}"
                )
