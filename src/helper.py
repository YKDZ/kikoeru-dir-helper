...
class ArchiveProcessor:
            ...
            with open(file_path, 'rb') as f:
                ...
                for signature, file_type in self.magic_signatures.items():
                    if header.startswith(signature):
                        logging.info(f"通过魔术字节检测到文件类型: {file_path} -> {file_type}")
                        return file_type
    ...
    def process_archive(self, archive_path: Path) -> bool:
        ...
        try:
            ...
            logging.info(f"📏 文件大小: {archive_path.stat().st_size / 1024 / 1024:.2f} MB")
            ...
            if actual_suffix != detected_type:
                ...
                if new_path != archive_path:
                    logging.info(f"🏷️ 文件扩展名不符，重命名: {archive_path.name} -> {new_path.name}")
                    archive_path.rename(new_path)
                    archive_path = new_path
            
            # 提取密码信息
            logging.info(f"🔐 分析密码信息...")
            password, clean_filename = self._extract_password_from_filename(archive_path)
            if password:
            ...
            if password:
                ...
                if new_path != archive_path:
                    logging.info(f"🏷️ 移除密码后重命名: {archive_path.name} -> {clean_filename}")
                    archive_path.rename(new_path)
                    archive_path = new_path
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                success = self._extract_archive_by_type(archive_path, temp_path, detected_type, password)
                if not success:
    ...
    def _extract_password_from_filename(self, file_path: Path) -> Tuple[Optional[str], str]:
        """
        从文件名中提取密码信息
        支持格式：
        ...
        if match:
            ...
            if remaining_passwords:
                ...
                clean_filename = re.sub(pattern, new_password_part, filename, flags=re.IGNORECASE)
            else:
    ...
    def _extract_archive_by_type(self, archive_path: Path, extract_to: Path, file_type: str, password: Optional[str] = None) -> bool:
        """
        根据检测到的文件类型解压文件
    ...
    def _extract_archive(self, archive_path: Path, extract_to: Path, password: Optional[str] = None) -> bool:
        """
        解压文件（保留兼容性）
        ...
        try:
            ...
            return self._extract_archive_by_type(archive_path, extract_to, suffix, password)
    ...
    def _extract_7z(self, archive_path: Path, extract_to: Path, password: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if password:
                ...
                with py7zr.SevenZipFile(archive_path, mode='r', password=password) as sz_ref:
    ...
    def _apply_rules(self, archive_path: Path, extracted_items: List[Path], temp_path: Path) -> bool:
        """
        应用处理规则
        ...
        try:
            ...
            for item in extracted_items:
                ...
                if detected_type:
                    ...
                    logging.info(f"  📦 发现内层压缩文件: {item.name} ({detected_type})")
                else:
            ...
            if archive_files and not other_files:
                logging.info(f"🔄 规则匹配: 全部为压缩文件，递归处理 {len(archive_files)} 个文件")
                ...
                for archive_file in archive_files:
                    ...
                    if has_remaining_passwords:
                        ...
                        _, remaining_filename = self._extract_password_from_filename(Path(original_name))
                        pass_pattern = r'\s+pass-([^.]+)'
                        match = re.search(pass_pattern, remaining_filename, re.IGNORECASE)
                        if match:
                            remaining_passwords = match.group(1).strip()
                            # 为内层文件添加剩余密码
                            file_stem = archive_file.stem
                            file_suffix = archive_file.suffix
                            new_name = f"{file_stem} pass-{remaining_passwords}{file_suffix}"
                            target_path = self.work_dir / new_name
                            logging.info(f"🔑 传递密码给内层文件: {new_name}")
                    
                    logging.info(f"📝 移动内层压缩文件: {archive_file.name} -> {target_path.name}")
            ...
            if non_dir_items:
                logging.info(f"📝 规则匹配: 包含非文件夹项目 ({len(non_dir_items)} 个文件)")
                return self._handle_mixed_content(archive_path, extracted_items)
            ...
            if len(folders) == 1 and not folders[0].name.startswith('RJ') and archive_name.startswith('RJ'):
            ...
            if len(folders) > 1 and any(not folder.name.startswith('RJ') for folder in folders):
            ...
            for i, folder in enumerate(folders, 1):
                ...
                if target_path.exists():
                    ...
                    while target_path.exists():
                        ...
                        counter += 1
                    logging.info(f"  ⚠️ 目标已存在，重命名: {original_name} -> {target_path.name}")
                
                logging.info(f"  {i}. 📁 移动RJ文件夹: {original_name} -> {target_path}")
    ...
    def _create_processing_log(self, target_dir: Path, archive_path: Path, reason: str) -> None:
        """创建处理日志文件"""
        try:
